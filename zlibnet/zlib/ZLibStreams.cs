...
namespace ZLibNet
{
	...
	public class DeflateStream : Stream
	{
		...
		{
            pLeaveOpen = leaveOpen;
            pStream = stream;
            pCompMode = compMode;

			int ret;
			if (pCompMode == CompressionMode.Compress)
				ret = ZLib.deflateInit(ref pZstream, level, WriteType);
			else
				ret = ZLib.inflateInit(ref pZstream, OpenType);
			...
			try
			{
				try
				{
					if (disposing) //managed stuff
					{
						...
						{
							//managed stuff
							if (pCompMode == CompressionMode.Compress && pSuccess)
							{
								Flush();
								//								this.pStream.Flush();
							}
