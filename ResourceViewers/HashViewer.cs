...
namespace ResourceExplorer
{
    public partial class HashViewer : Form
    {
        ...
        private byte[] ConvertString(string value)
        {
            byte[] data = null;

            if (_CheckHex.Checked)
            {
        ...
        private void OnCheckHexChecked(object sender, EventArgs e)
        {
            try
            {
                string value = textBoxSrc.Text;
                bool hex = Helpers.IsHexString(value);

                if (hex && _CheckHex.Checked || !hex && !_CheckHex.Checked)
                {
                    return;
                }
                ...
                {
                    textBoxSrc.Text = _Encoding.GetString(Fast.FromHexString(value));
                }
                else
                {
                    textBoxSrc.Text = Fast.ToHexString(_Encoding.GetBytes(value));
                }
            }
            catch (Exception ex)
            {
