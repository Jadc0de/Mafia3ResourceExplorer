...
namespace ManagedBass.Misc
{
	...
	public sealed class Visuals
	{
        ...
        public int FFTIndex2Frequency(int index, int length, int samplerate)
        {
        ...
        public int GetFrequencyFromPosX(int x, int samplerate)
		{
            ...
            return FFTIndex2Frequency(num, this.MaxFFTData, samplerate);
        }
        ...
        public int FFTFrequency2Index(int frequency, int length, int samplerate)
        {
        ...
        public float DetectFrequency(int channel, int freq1, int freq2, bool linear)
		{
			...
			try
			{
				if (this.GetFFTData(channel, (int)this.MaxFFT) > 0)
				{
                    ChannelInfo bASS_CHANNELINFO = new ChannelInfo();
                    if (Bass.ChannelGetInfo(channel, out bASS_CHANNELINFO))
					{
						int num3 = FFTFrequency2Index(freq1, this.MaxFFTData, bASS_CHANNELINFO.Frequency); // Utils
                        int num4 = FFTFrequency2Index(freq2, this.MaxFFTData, bASS_CHANNELINFO.Frequency); // Utils
                        num2 = num4 - num3 + 1;
			...
			try
			{
				if (this.GetFFTData(channel, (int)this.MaxFFT) > 0)
				{
					...
					if (Bass.ChannelGetInfo(channel, out bASS_CHANNELINFO))
					{
						...
						result = FFTIndex2Frequency(index, this.MaxFFTData, bASS_CHANNELINFO.Frequency); // Utils
                    }
				}
			}
			catch
		...
		private int GetFFTData(int channel, int length)
		{
			if (this._channelIsMixerSource)
			{
                ...
                new FormatException("return BassMix.BASS_Mixer_ChannelGetData(channel, this._fft, length);");
			}

			return Bass.ChannelGetData(channel, this._fft, length);
		}
