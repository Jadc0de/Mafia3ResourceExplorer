...
namespace ManagedBass
{
    ...
    public sealed class Utils
    {
        ...
        public static string ElapsedToHMS(long milliseconds)
        {
        ...
        {
            return (int)(qWord >> 32 & 0xFFFFFFFFL);
        }
        ...
        public static string TSHMSF(double seconds)
        {
		...
		public static string IntPtrAsStringUtf8orLatin1(IntPtr utf8Ptr, out int len)
		{
			...
			if (utf8Ptr != IntPtr.Zero)
			{
				...
				if (len != 0)
				{
                    ...
                    string text = false ? Encoding.Default.GetString(array, 0, len) : Encoding.GetEncoding("latin1").GetString(array, 0, len);
                    try
					{
		...
		public static string BASSAddOnGetSupportedFileExtensions(Dictionary<int, string> plugins, bool includeBASS)
		{
			...
			if (plugins != null)
			{
				foreach (string current in plugins.Values)
				{
					stringBuilder.Append(Utils.BASSAddOnGetSupportedFileExtensions(current));
					if (stringBuilder.Length > 0)
					{
						stringBuilder.Append(";");
					}
		...
		public static string BASSAddOnGetSupportedFileFilter(Dictionary<int, string> plugins, string allFormatName, bool includeBASS)
		{
			...
			if (plugins != null)
			{
				foreach (string expr_10C in plugins.Values)
				{
					text = Utils.BASSAddOnGetSupportedFileName(expr_10C);
					text2 = Utils.BASSAddOnGetSupportedFileExtensions(expr_10C);
					if (!string.IsNullOrEmpty(text) && !string.IsNullOrEmpty(text2))
					{
						stringBuilder.Append(string.Concat(new string[]
						{
		...
		public static string BASSAddOnGetSupportedFileFilter(Dictionary<int, string> plugins, string allFormatName, bool includeBASS, Dictionary<string, string> extra)
		{
			...
			if (extra != null)
			{
				foreach (KeyValuePair<string, string> current in extra)
				{
					text = current.Key;
					text2 = current.Value;
					if (!string.IsNullOrEmpty(text) && !string.IsNullOrEmpty(text2))
		...
		public static string BASSAddOnGetPluginFileFilter(Dictionary<int, string> plugins, string allFormatName, bool includeBASS)
		{
			...
			if (includeBASS)
			{
				...
				{
					BASS_PLUGINFORM expr_32 = formats[i];
					text = expr_32.name;
					text2 = expr_32.exts;
					if (!string.IsNullOrEmpty(text) && !string.IsNullOrEmpty(text2))
					{
			...
			if (plugins != null)
			{
				using (Dictionary<int, string>.KeyCollection.Enumerator enumerator = plugins.Keys.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						BASS_PLUGINFORM[] formats = Bass.BASS_PluginGetInfo(enumerator.Current).formats;
						for (int i = 0; i < formats.Length; i++)
						{
							BASS_PLUGINFORM expr_C1 = formats[i];
							text = expr_C1.name;
							text2 = expr_C1.exts;
							if (!string.IsNullOrEmpty(text) && !string.IsNullOrEmpty(text2))
							{
		...
		public static bool DetectCuePoints(string filename, float blockSize, ref double cueInPos, ref double cueOutPos, double dBIn, double dBOut, int findZeroCrossing)
		{
			...
			bool arg_29_0 = Utils.DetectCuePoints(num, blockSize, ref cueInPos, ref cueOutPos, dBIn, dBOut, findZeroCrossing);
			if (num != 0)
			{
		...
		public static float GetNormalizationGain(string filename, float blockSize, double startpos, double endpos, ref float peak)
		{
			...
			float result = 1f;
			long num2 = 0L;
			long num3 = Bass.ChannelGetLength(num);
			if (startpos < 0.0)
			{
				startpos = 0.0;
			}
			...
			if (endpos > startpos && endpos <= Bass.ChannelBytes2Seconds(num, num3))
			{
				num2 = Bass.ChannelSeconds2Bytes(num, startpos);
				num3 = Bass.ChannelSeconds2Bytes(num, endpos);
			}
			if (num2 > 0L)
			{
				Bass.ChannelSetPosition(num, num2);
			}
			int num4 = (int)Bass.ChannelSeconds2Bytes(num, (double)blockSize);
			float[] array = new float[num4 / 4];
			float num5 = 0f;
			while (num2 < num3)
			{
				int num6 = Bass.ChannelGetData(num, array, num4 | 1073741824);
				if (num6 < 0)
				{
					num2 = num3;
				}
